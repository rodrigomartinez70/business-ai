"""
Motor Riesgo Tributario (horizontal) — inconsistencias, fiscalización y alertas.

Agnóstico al vertical: recibe `ingresos_fn(conn, ini, fin)` para el ratio
gasto/ingreso. El resto (documentos pendientes, gastos sin comprobante, deuda de
IVA, score) es común.
"""
...
async def calcular_riesgo(conn, hasta: date, iva: dict, ingresos_fn, uf: float | None = None) -> dict:
    ...
    gastos_30 = to_float(await conn.fetchval(
        ...
        hace_30, hasta) or 0)
    ingresos_30 = await ingresos_fn(conn, hace_30, hasta)
